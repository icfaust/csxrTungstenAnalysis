...
def fullObjectiveLog(fz, val, c_W, M):
    ...
    idx = temp != 0       
    #print(fz)
    #print(scipy.sum(pow(scipy.log(val[idx]) - scipy.log(temp[idx]), 2))/2.)
    #print(' ')
...
def objectiveLogJac(fz, val, c_W, M):
    ...
    # I am going to do this in a for loop, because M is huge and I don't want to overload on memory
    for i in xrange(len(fz)):
        output[i] = scipy.sum((scipy.log(temp[idx]) - scipy.log(val[idx]))*c_W[idx]*M[idx,i]/temp[idx])
    print(fz)
    print(output)
    print(' ')
    return output
...
def objectiveLogJac2(fz, val, c_W, M):
    ...
    # I am going to do this in a for loop, because M is huge and I don't want to overload on memory
    output = scipy.dot((scipy.log(temp[idx]) - scipy.log(val[idx]))*c_W[idx]/temp[idx],M[idx])
    #print(fz)
    #print(output)
    #print(' ')
    return output

def objectiveLogHess(fz, val, c_W, M):
    output = scipy.zeros((len(fz),len(fz)))
    temp = c_W*scipy.dot(M, fz)
    
    idx = temp != 0

    # I am going to do this in a for loop, because M is huge and I don't want to overload on memory
    for j in xrange(len(fz)):
        for i in xrange(len(fz)):
            output[j,i] = scipy.sum((1 - scipy.log(temp[idx]) + scipy.log(val[idx]))*pow(c_W[idx],2)*M[idx, i]*M[idx, j]/pow(temp[idx],2))
    return output


def objectiveLogTest(fz, val, c_W, M):
    output = scipy.zeros((len(fz),len(fz)))
    temp = c_W*scipy.dot(M, fz)
    ...
    for j in xrange(len(fz)):
        for i in xrange(len(fz)):
            output[j,i] = scipy.sum(pow(scipy.log(temp[idx]) - scipy.log(val[idx]),2)*pow(c_W[idx],2)*M[idx, i]*M[idx, j]/pow(temp[idx],2))
    return output
...
def regFullObjLog3(fz, val, c_W, M, lam, mat): 
    output = fullObjectiveLog(fz, val, c_W, M)/len(val)
    #print(fz)
    #regularizer maintaining positive solutions
    reg = dotPositive(mat, scipy.log(abs(fz)))
    output += lam*scipy.sum(pow(reg,2))/2./len(val)
...
def regFullObjLogJac(fz, val, c_W, M, lam, mat):
    output = objectiveLogJac2(fz, val, c_W, M)/len(val)
...
def regFullObjLogJac3(fz, val, c_W, M, lam, mat): #scale values exponentially to prevent negative values
    output = objectiveLogJac2(fz, val, c_W, M)/len(val)

    #regularizer maintaining positive solutions
    reg = dotPositive(mat, scipy.log(abs(fz)))/fz
    output += lam*reg/len(val)
  
    return output


def regFullObjLogHess(fz, val, c_W, M, lam, mat):
    output = objectiveLogHess(fz, val, c_W, M)/len(val)

    output += lam*mat/len(val)
...
def fitDataBFGSM(M, val, c_w_l, init=None, lims=[2.75e3,2e4], nozero=True, k=3e34, lam=1., start=36, name='W_Abundances_grid_puestu_adpak_fitscaling_74_0.00000_5.00000_1000_idlsave'): #init is the three initial values of the gaussian needed to fit the data
    ...
    init = 1e-5*scipy.ones(init.shape)
    ...
    bndarray = scipy.ones((len(init[idx]),2))
    bndarray[:,0] = 1e-50
    bndarray[:,1] = 1e10
    reg = gen2Eye(init[idx])

    y = time.time()
    output = scipy.optimize.minimize(regFullObjLog,
                                     init[idx],
                                     args=(val, c_w_l, M[:,idx]/k, lam, reg),
                                     jac=regFullObjLogJac,
                                     bounds=bndarray)
    print(time.time()-y)
    return output

def fitDataBFGSM2(M, val, c_w_l, init=None, lims=[2.75e3,2e4], nozero=True, k=3e34, lam=1., name='W_Abundances_grid_puestu_adpak_fitscaling_74_0.00000_5.00000_1000_idlsave'): #init is the three initial values of the gaussian needed to fit the data
    """ function for determining the optimal fit given the desired parabolic regularization"""

    #intialize start position
    temp = scipy.io.readsav(name)
    init = temp['abundance'][:,38]
    reg = gen2Diff(init)
    ...
    output = scipy.optimize.minimize(fullObjectiveLog,
                                     init[idx],
                                     args=(val, c_w_l, M[:,idx]/k),
                                     jac=objectiveLogJac2,
                                     bounds=bndarray)
    print(time.time()-y)
    return output

def fitDataBFGSM3(M, val, c_w_l, init=None, lims=[2.75e3,2e4], nozero=True, k=3e34, lam=1., start=36, name='W_Abundances_grid_puestu_adpak_fitscaling_74_0.00000_5.00000_1000_idlsave'): #init is the three initial values of the gaussian needed to fit the data
    """ function for determining the optimal fit given the desired parabolic regularization"""
    ...
    bndarray[:,0] = 1e-10
    bndarray[:,1] = 1e10
    reg = gen2Diff(init[idx])

    y = time.time()
    output = scipy.optimize.minimize(regFullObjLog,
                                     init[idx],
                                     args=(val, c_w_l, M[:,idx]/k, lam, reg),tol=1e-8,
                                     jac=regFullObjLogJac,
                                     bounds=bndarray)
    print(time.time()-y)
    return output
...
def fitDataBFGSM4(M, val, c_w_l, init=None, lims=[2.75e3,2e4], nozero=True, k=3e34, lam=1., start=36, name='W_Abundances_grid_puestu_adpak_fitscaling_74_0.00000_5.00000_1000_idlsave'): #init is the three initial values of the gaussian needed to fit the data
    """ function for determining the optimal fit given the desired parabolic regularization"""
    ...
    y = time.time()
    output = scipy.optimize.minimize(regFullObjLog3,
                                     init[idx],
                                     args=(val, c_w_l, M[:,idx]/k, lam, reg),
                                     jac=regFullObjLogJac3,
                                     bounds=bndarray)
    print(time.time()-y)
    return output

def fitDataBFGSM5(M, val, c_w_l, init=None, lims=[2.75e3,2e4], nozero=True, k=3e34, lam=1., start=36, name='W_Abundances_grid_puestu_adpak_fitscaling_74_0.00000_5.00000_1000_idlsave'): #init is the three initial values of the gaussian needed to fit the data
    """ function for determining the optimal fit given the desired parabolic regularization"""
...
def findLam(M, val, c_w_l, init=None, lims=[2.2e3,9e3], nozero=True, k=3e34, lam=[0.,6.], pts=1001, loc='lamFits36.p'):
    
    output = {}
    lams = scipy.logspace(lam[0],lam[1],pts)

    for i in xrange(len(lams)):
        print(i)
        output[i] = [i,fitDataBFGSM(M, val, c_w_l, init=init, lims=lims, nozero=nozero, k=3e34, lam=lams[i])]
...
def bootstrapLam(M, val, c_w_l, init=None, lims=[2.2e3,9e3], nozero=True, k=3e34, lam=4149.54, pts=1001, loc='bootstrap_lamFitsflatEye.p'):
    ...
    for i in xrange(pts):
        print(i)
        #randomly sample
        idx = numpy.random.choice(temp,temp.shape)
        output[i] = [i,fitDataBFGSM(M[idx], val[idx], c_w_l[idx], init=init, lims=lims, nozero=nozero, k=3e34, lam=lam)]
...
def findLamDiff(M, val, c_w_l, init=None, lims=[2.2e3,9e3], nozero=True, k=3e34, lam=[0.,6.], pts=1001, loc='lamFitsDiff36.p'):
    ...
    for i in xrange(len(lams)):
        print(i)
        output[i] = [i,fitDataBFGSM3(M, val, c_w_l, init=init, lims=lims, nozero=nozero, k=3e34, lam=lams[i])]
...
def bootstrapLamDiff(M, val, c_w_l, init=None, lims=[2.2e3,9e3], nozero=True, k=3e34, lam=4149.54, pts=1001, loc='bootstrap_lamFitsflatDiff.p'):
    ...
    for i in xrange(pts):
        ...
        #randomly sample
        idx = numpy.random.choice(temp,temp.shape)
        output[i] = [i,fitDataBFGSM3(M[idx], val[idx], c_w_l[idx], init=init, lims=lims, nozero=nozero, k=3e34, lam=lam)]
...
def findLam2(M, val, c_w_l, init=None, lims=[2.2e3,9e3], nozero=True, k=3e34, lam=[0.,6.], pts=1001, loc='lamFitsExp36.p'):
    ...
    for i in xrange(len(lams)):
        print(i)
        output[i] = [i,fitDataBFGSM4(M, val, c_w_l, init=init, lims=lims, nozero=nozero, k=3e34, lam=lams[i])]
...
def findLam2Diff(M, val, c_w_l, init=None, lims=[2.2e3,9e3], nozero=True, k=3e34, lam=[0.,6.], pts=1001, loc='lamFitsExp36.p'):
    ...
    for i in xrange(len(lams)):
        print(i)
        output[i] = [i,fitDataBFGSM5(M, val, c_w_l, init=init, lims=lims, nozero=nozero, k=3e34, lam=lams[i])]
...
def visualizeR2(minval,loc= '/afs/ipp-garching.mpg.de/home/i/ianf/python/csxr/lamFitsEye50.p', lims=[2.2e3,9e3], lims2=[2.3e3,7e3],alpha=.8):
    """ goodness of fit vs charge state fractional abundance with expected PEC """
    ...
    for j in chargestates:
        ...
        differ = scipy.log(PEC[idx]*fz[idx])-scipy.log(data2[minval][1].x[idx2])
        differ2 = scipy.log(fz[idx])-scipy.log(data2[minval][1].x[idx2])
        
        if not noPEC:
            rsquared[j-1]= 1- scipy.var(differ)/scipy.var(scipy.log(data2[minval][1].x[idx2])) 
      
        rsquared2[j-1]= 1- scipy.var(differ2)/scipy.var(scipy.log(data2[minval][1].x[idx2]))
    ...
    plt.plot(chargestates,rsquared2,color='#228B22',marker='o',markersize=10,lw=0,label=r'$ f_Z$')
    plt.xlim([42,48])
    plt.xticks(scipy.mgrid[42:49:1])
    plt.ylim([0,1])

    plt.xlabel(r'charge state')
    plt.ylabel(r'$R^2$ of $\log$-$\log$ fit')
    leg = plt.legend(loc=4,numpoints=1,fontsize=18,title=r'\underline{\hspace{.5ex} max$(R^2)=.941$ \hspace{.5ex}}')
    plt.setp(leg.get_title(),fontsize=14)
    plt.subplots_adjust(bottom=.12)
    plt.show()
...
def regSolutionExample(minval,loc='/afs/ipp-garching.mpg.de/home/i/ianf/python/csxr/lamFitsEye50.p',k=3e34, lims=[2.2e3,9e3], lims2=[2.3e3,7e3], alpha=.8,err=None):
    ...
    #temp = c_w*scipy.dot(M[:,idxin]/k, fz)
    
    #idxt = temp != 0
    #div = len(temp[idxt])
    #print(len(temp[idxt]),len(val))

    #sigma2 = scipy.var(scipy.log(temp[idxt]) - scipy.log(val[idxt]))
    
    #sigma2 = 2*fullObjectiveLog(fz, val, c_w, M[:,idxin]/k)/div
    #jac = 2*objectiveLogJac2(fz, val, c_w, M[:,idxin]/k)/div

    
    sigma = scipy.sqrt(data[minval][1].fun*scipy.diag(data[minval][1].hess_inv.todense()))
    sigma = scipy.exp(abs(scipy.log(sigma))) #force to greater than one
    #sigma = scipy.sqrt(scipy.diag(data[minval][1].hess_inv.todense()))
    #print(sigma)
    ...
    plt.loglog(Te[idx],data[minval][1].x[idx2]/data[minval][1].x[idx2].max(),lw=3,color='#0E525A',label='_nolabel_')
...
def regSolutionExample2(minval,loc='/afs/ipp-garching.mpg.de/home/i/ianf/python/csxr/lamFitsEye50.p',k=3e34, lims=[2.2e3,9e3], lims2=[2.3e3,7e3], alpha=.8,err=None):
...
def regSolutionExample3(minval,k=3e34, lims=[2.2e3,9e3], lims2=[2.3e3,7e3], alpha=.8,err=None):
    ...
    plt.loglog(Te[idx],minval.x[idx2]/minval.x[idx2].max(),lw=3,color='#0E525A',label='_nolabel_')
...
def regSolutionExampleVal7(k=3e34, lims=[2.2e3,9e3], lims2=[2.3e3,7e3], alpha=.8,err=None,err2=None):
    ...
    fz = data[minval][1].x[idx2]

    minval2 = 649
    loc2 = 'lamFitsDiff36.p'
    datas = pickle.load(open(loc2,'rb')) 

    fz2 = datas[minval2][1].x[idx2]

    minval3 = [2,5]
    loc3 = 'svmcrossvalid.p'
    datasvm = pickle.load(open(loc3,'rb')) 

    fz3 = scipy.dot(scipy.linalg.inv(svmmodel.genHatM(lims)),datasvm[1][minval3[0]][minval3[1]])[idx2]
    print(len(fz3),len(fz2))
    ...
    plt.loglog(Te[idx],fz/fz.max(),lw=3,color='#0E525A',label='Tikh')#label='_nolabel_')
    plt.loglog(Te[idx],fz2/fz2.max(),lw=3,color='#228b22',label='$2^{nd}$ O.')#label='_nolabel_')
    plt.loglog(Te[idx],fz3/fz3.max(),lw=3,color='darkorange',label='SVM')#label='_nolabel_')
    ...
    plt.ylabel(r'$\tilde{f}_Z$/max$(\tilde{f}_Z)$')
    plt.xticks([2e3,3e3,4e3,5e3,6e3,7e3,8e3,9e3],['2','3','4','5','6','7','8','9'])
    leg = plt.legend(loc=8,fontsize=18,title=r'\underline{\hspace{.5em} \emph{solid} - \emph{$\tilde{f}_Z$ fits}\hspace{.5em} }')   
    plt.setp(leg.get_title(),fontsize=14)
    plt.subplots_adjust(bottom=.12)
    plt.show()
...
def regSolutionExampleVal11(k=3e34, lims=[2.2e3,9e3], lims2=[2.3e3,7e3], alpha=.8,err=None,err2=None):
    ...
    minval = 720
    loc = 'vall11lamFitsEyeflat.p'
    data = pickle.load(open(loc,'rb')) 

    fz = data[minval][1].x[idx2]

    minval2 = 588
    loc2 = 'vall11lamFitsDiffflat.p'
    datas = pickle.load(open(loc2,'rb')) 

    fz2 = datas[minval2][1].x[idx2]

    minval3 = [2,4]
    loc3 = 'svmcrossvalidval11.p'
    datasvm = pickle.load(open(loc3,'rb')) 
    ...
    plt.loglog(Te[idx],fz/fz.max(),lw=3,color='#0E525A',label='_nolabel_')
    #plt.loglog(Te[idx],fz2/fz2.max(),lw=3,color='#228b22',label='2$^{nd}$ O.')#label='_nolabel_')
    #plt.loglog(Te[idx],fz3/fz3.max(),lw=3,color='darkorange',label='SVM')#label='_nolabel_')
