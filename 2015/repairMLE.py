...
def run(shots=[32812,34995], idx=0, name='shots2015'):
...
def rerun(name='shots2015',startidx=0):
    conn = sqlite3.connect(_tablename)
...
def addColumn(name='c_w_qc', table='shots2015', datatype='FLOAT'):
    conn = sqlite3.connect(_tablename)
    c = conn.cursor()
...
def addGIWdata(conn, shot, name='c_W', name2='c_W_l', table='shots2015'):
    ...
    c = conn.cursor()
    points = scipy.atleast_1d(scipy.squeeze((c.execute('SELECT id from '+table+' where shot='+str(shot))).fetchall()))
    time = scipy.atleast_1d(scipy.squeeze((c.execute('SELECT time from '+table+' where shot='+str(shot))).fetchall()))
    ...
    for i in xrange(len(points)):
        c.execute('UPDATE '+table+' SET "c_W"='+str(output[i])+' WHERE id='+str(points[i]))
        c.execute('UPDATE '+table+' SET "c_W_l"='+str(output2[i])+' WHERE id='+str(points[i]))

    conn.commit()
    #print(shot),


def findShots(conn, table='shots2015'):
    c = conn.cursor()
...
def changeData(shotlist, data, idx, name='shots2015'):
...
def deleteData(idx, name='shots2015'):

    conn = sqlite3.connect(_tablename)
    curs= conn.cursor()
    text = 'DELETE FROM '+name+' WHERE id='

    for i in idx:
        curs.execute(text+str(i))
